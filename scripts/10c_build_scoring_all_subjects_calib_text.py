#!/usr/bin/env python
"""Build all-subject scoring calibration text from local xlsx files."""
...
def _answer_samples_for_question(
    ...
    *,
    ...
    candidates: list[ScoringTextSample] = []
    for answer_row in _row_dicts(read_xlsx_first_sheet(answer_file)):
    ...
    if rows_per_file is not None and rows_per_file > 0 and len(candidates) > rows_per_file:
        candidates = rng.sample(candidates, rows_per_file)
    return candidates
    ...
    *,
    ...
    for question_row in question_rows:
        ...
        if not answer_file.exists():
            print(f"[build-scoring-all] skip missing answer file: {answer_file}")
            continue
        file_samples = _answer_samples_for_question(
            question_row,
            answer_file,
            rng=rng,
        ...
        print(f"[build-scoring-all] {file_id} ({subject}): {len(file_samples)} samples")
    return samples


def main() -> None:
    parser = argparse.ArgumentParser(description="Build all-subject scoring calibration text for projector training.")
    parser.add_argument("--question-file", default="pingfen/题目信息.xlsx")
    parser.add_argument("--answer-dir", default="pingfen/评分数据")
    parser.add_argument("--output", default="data/scoring_all_train.txt")
    parser.add_argument("--rows-per-file", type=int, default=200, help="Random samples per question file. Use 0 for all valid rows.")
    parser.add_argument("--max-samples", type=int, default=None, help="Optional global random cap after per-file sampling.")
    parser.add_argument("--include-subjects", nargs="*", default=None, help="Optional subject allowlist, e.g. 历史 语文 政治.")
    ...
    rows_per_file = None if args.rows_per_file is not None and args.rows_per_file <= 0 else args.rows_per_file
    samples = collect_samples(
        Path(args.question_file),
        Path(args.answer_dir),
        rows_per_file=rows_per_file,
        include_subjects=_split_items(args.include_subjects),
        exclude_subjects=_split_items(args.exclude_subjects),
    ...
    print(f"[build-scoring-all] wrote {len(samples)} samples to {output}")
